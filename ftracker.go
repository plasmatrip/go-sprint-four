...
func ShowTrainingInfo(action int, trainingType string, duration, weight, height float64, lengthPool, countPool int) string {
	outputString := "Тип тренировки: %s\nДлительность: %.2f ч.\nДистанция: %.2f км.\nСкорость: %.2f км/ч\nСожгли калорий: %.2f\n"
	switch {
	case trainingType == "Бег":
		distance := distance(action)
		speed := meanSpeed(action, duration)
		calories := RunningSpentCalories(action, weight, duration)
		return fmt.Sprintf(outputString, trainingType, duration, distance, speed, calories)
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	distance := meanSpeed(action, duration) * kmhInMsec
	heightInM := height / cmInM
	return (walkingCaloriesWeightMultiplier*weight + (math.Pow(distance, 2)/heightInM)*walkingSpeedHeightMultiplier*weight) * duration * minInH
}
